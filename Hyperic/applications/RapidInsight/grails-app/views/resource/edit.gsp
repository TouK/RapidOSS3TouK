            ...
            <table>
                <tbody>
                    ...
                    <tr class="prop">
                        ...
                        <td valign="top" class="value ${hasErrors(bean:resource,field:'hypericEvents','errors')}">
                            
<ul>
<g:each var="h" in="${resource?.hypericEvents?}">
    <li style="margin-bottom:3px;">
        <g:link controller="hypericEvent" action="show" id="${h.id}">${h}</g:link>
        <g:link class="delete" action="removeRelation" params="['id':resource?.id, 'relationName':'hypericEvents', 'relatedObjectId':h.id]"></g:link>
    </li>
</g:each>
</ul>
<g:link params="['id':resource?.id, 'relationName':'hypericEvents']" action="addTo">Add HypericEvent</g:link>
