...
target ('watchDirs': "Start test watching threads") {
    def watchedDirs = new File("watchedDirs.txt");
    if(!watchedDirs.exists()) throw new Exception("create watchedDirs.txt in ${System.getProperty("base.dir")}");
    def excludedDirs = [".svn":".svn",
                "reports":"reports"]
    watchedDirs.eachLine {String line->
        def dirString = StringUtils.split(line, ",");
        def watchedDir = new File(dirString[0].trim())
        def destDir = new File(dirString[1].trim())
        FileUtils.copyDirectory (watchedDir, destDir);
        dirListeners = new DirListener(watchedDir, destDir, excludedDirs);
    }
}
