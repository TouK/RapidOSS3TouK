...
class ModelGeneratorInvalidModelTest extends RapidCmdbTestCase {
    ...
    public void testIfPropertytypeIsNotSpecified()
    {
        def prop1 = [name: "prop1", type: "invalid", blank: false, defaultValue: "1"]
        String modelName = "Model1";
        def modelXml = createModel(modelName, null, [prop1], [prop1], []);
        try
        {
            ModelGenerator.getInstance().generateModels([modelXml])
            fail("Should throw exception since property type is invalid");
        } catch (ModelGenerationException e)
        {
            assertEquals(ModelGenerationException.invalidModelpropertyType(modelName, prop1.name, prop1.type).getMessage(), e.getMessage());
        }
    }
    ...
    public void testThrowsExceptionIfRelationTypeIsInvalid()
    {
        String modelName = "Model";
        String relatedModelName = "RelatedModel";
        def ds1 = [name: "RCMDB", keyMappings: []]
        def rel1 = [name: "rel1", reverseName: "revrel1", toModel: relatedModelName, cardinality: "invalid", reverseCardinality: ModelGenerator.RELATION_TYPE_ONE, isOwner: true];
        def revrel1 = [name: "revrel1", reverseName: "rel1", toModel: modelName, cardinality: ModelGenerator.RELATION_TYPE_ONE, reverseCardinality: ModelGenerator.RELATION_TYPE_ONE, isOwner: false];
        def modelXml1 = createModel(modelName, null, [ds1], [], [], [rel1]);
        def modelXml2 = createModel(relatedModelName, null, [ds1], [], [], [revrel1]);
        try
        {
            ModelGenerator.getInstance().generateModels([modelXml1, modelXml2])
            fail("Should throw exception since relation cardinality is invalid");
        } catch (ModelGenerationException e)
        {
            assertEquals(ModelGenerationException.invalidModelRelationType(modelName, rel1.name, rel1.cardinality).getMessage(), e.getMessage(),);
        }
    }
