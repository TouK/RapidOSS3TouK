...
class HistoricalNotificationsTagLib {
    ...
    def historicalNotifications = {attrs, body ->
        ...
        out << TreeGridTagLib.fTreeGrid(id: "filterTree", url: "script/run/queryList?format=xml&type=historicalnotification", rootTag: "Filters",
        ...
        out << RFormTagLib.fForm(id: "filterDialog", width: "35em", createUrl: "script/run/createQuery?queryType=historicalnotification", editUrl: "script/run/editQuery?queryType=historicalnotification",
                saveUrl: "searchQuery/save?format=xml&type=historicalnotification", updateUrl: "searchQuery/update?format=xml&type=historicalnotification", onSuccess: "refreshQueriesAction",
                """
                  <div>
        ...
        out << RFormTagLib.fForm(id: "filterGroupDialog", width: "30em", saveUrl: "searchQueryGroup/save?format=xml&type=historicalnotification",
                updateUrl: "searchQueryGroup/update?format=xml&type=historicalnotification", onSuccess: "refreshQueriesAction",
                """
                  <div>
    ...
    def hnSearchResults = {attrs, body ->
        out << TagLibUtils.getConfigAsXml("HnSearchResults", attrs, [], body());
    }
    def hnSearchResult = {attrs, body ->
        def validAttrs = ["alias", "properties", "emphasizeds"]
        out << TagLibUtils.getConfigAsXml("HnSearchResult", attrs, validAttrs);
    }

    def hnConversions = {attrs, body ->
        out << TagLibUtils.getConfigAsXml("HnConversions", attrs, [], body());
    }
    def hnConversion = {attrs, body ->
        def validAttrs = ["type", "format", "property", "function", "mapping"]
        out << TagLibUtils.getConfigAsXml("HnConversion", attrs, validAttrs);
    }
