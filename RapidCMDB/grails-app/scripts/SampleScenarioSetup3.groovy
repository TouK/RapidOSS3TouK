...
rcmdbModelDatasource = ModelDatasource.create(datasource:rcmdbDS, master:true);
//def resource1DS= SingleTableDatabaseDatasource.findByName("dsResource1");
//def resource2DS= SingleTableDatabaseDatasource.findByName("dsResource2");
...
def operationalstate = ModelProperty.create(name:"operationalstate", type:ModelProperty.stringType, blank:false, lazy:true, propertySpecifyingDatasource:dsname);
resource = constructModel(resource, [name, displayname, dsname, operationalstate], [rcmdbModelDatasource, resource1ModelDatasource, resource2ModelDatasource], keyMappings);
resource.refresh();
...
def ipaddress = ModelProperty.create(name:"ipaddress", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"ip", propertyDatasource:deviceModelDatasource);
...
def memberof = ModelProperty.create(name:"memberOf", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"memberof", propertyDatasource:linkModelDatasource);
...
def severity = ModelProperty.create(name:"severity", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"Severity", propertyDatasource:eventModelDatasource);
def ack = ModelProperty.create(name:"ack", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"Acknowledged", propertyDatasource:eventModelDatasource);
def owner = ModelProperty.create(name:"owner", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"Owner", propertyDatasource:eventModelDatasource);
def description = ModelProperty.create(name:"description", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"Description", propertyDatasource:eventModelDatasource);
def lastOccured = ModelProperty.create(name:"lastOccured", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"LastOccuredAt", propertyDatasource:eventModelDatasource);
def lastChanged = ModelProperty.create(name:"lastChanged", type:ModelProperty.stringType, blank:false, lazy:true, nameInDatasource:"LastChangedAt", propertyDatasource:eventModelDatasource);
...
createRelation(resource, event, "hasEvents", "occurredAt", ModelRelation.ONE, ModelRelation.MANY);
createRelation(service, resource, "usesResources", "usedForServices", ModelRelation.MANY, ModelRelation.MANY);

ModelGenerator.getInstance().generateModel(device);
ModelGenerator.getInstance().generateModel(link);

return "Successfully created model classes for Scenario 3!";
