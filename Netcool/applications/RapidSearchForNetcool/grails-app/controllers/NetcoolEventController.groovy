...
class NetcoolEventController {
    ...
    def delete = {
        ...
        if(netcoolEvent) {
            try{
                ...
                addError("default.couldnot.delete", [NetcoolEvent, netcoolEvent])
    ...
    def addTo = {
        ...
        if(!netcoolEvent){
            ...
            if(relationName){
                def otherClass = DomainClassUtils.getStaticMapVariable(NetcoolEvent, "relations")[relationName].type;
    ...
    def removeRelation = {
        ...
        if(!netcoolEvent) {
            ...
            def otherClass = com.ifountain.rcmdb.domain.util.DomainClassUtils.getStaticMapVariable(NetcoolEvent, "relations")[relationName].type;
            if(otherClass){
                def res = otherClass.metaClass.invokeStaticMethod(otherClass, "get", params.relatedObjectId.toLong());
                if(res){
                      def relationMap = [:];
