            ...
            <table>
                <tbody>
                    ...
                    <tr class="prop">
                        ...
                        <td valign="top" class="value ${hasErrors(bean:rsPort,field:'memberOfGroup','errors')}">
                            
<ul>
<g:each var="m" in="${rsPort?.memberOfGroup?}">
    <li style="margin-bottom:3px;">
        <g:link controller="rsGroup" action="show" id="${m.id}">${m}</g:link>
        <g:link class="delete" action="removeRelation" params="['id':rsPort?.id, 'relationName':'memberOfGroup', 'relatedObjectId':m.id]"></g:link>
    </li>
</g:each>
</ul>
<g:link params="['id':rsPort?.id, 'relationName':'memberOfGroup']" action="addTo">Add RsGroup</g:link>
