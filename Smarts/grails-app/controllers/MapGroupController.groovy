...
class MapGroupController {
        ...
        if(mapGroup) {
            try {
                ...
                withFormat { 
                    ...
                    xml {render(text: ControllerUtils.convertSuccessToXml("MapGroup ${params.id} deleted"), contentType: "text/xml")}
                }

            }
            catch(e){
                addError("default.couldnot.delete", [MapGroup, mapGroup])
                withFormat {
                    html {
                        flash.errors = errors;
    ...
    def update = {
        def mapGroup = MapGroup.get( [id:params.id] )
        if(mapGroup) {
            mapGroup.update(ControllerUtils.getClassProperties(params, MapGroup));
            if(!mapGroup.hasErrors()) {
                withFormat {
                    ...
                    xml {render(text: ControllerUtils.convertSuccessToXml("MapGroup ${params.id} updated"), contentType: "text/xml")}
                }
            }
            else {
                withFormat {
    ...
    def save = {
       params["username"] = session.username;
        def mapGroup = MapGroup.add(ControllerUtils.getClassProperties(params, MapGroup))
        if (!mapGroup.hasErrors()) {
            withFormat {
                html {
                ...
                xml {render(text: ControllerUtils.convertSuccessToXml("MapGroup ${params.name} created"), contentType: "text/xml")}
            }
        }
        else {
            withFormat {
