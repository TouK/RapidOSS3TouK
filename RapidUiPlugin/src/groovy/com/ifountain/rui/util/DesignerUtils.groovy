...
class DesignerUtils {
    ...
    {
        generateXml(components, builder, metadataGetter, 1);
    }
    private static void generateXml(List components, MarkupBuilder builder, Closure metadataGetter, int autoGenerateId)
    {
                ...
                children.each {child ->
                    if (child.metaData == null)
                    {
                        ...
                        try {
                            ...
                            generateXml(childObjects, builder, metadataGetter, autoGenerateId);
                        } catch (groovy.lang.MissingPropertyException e) {}
                    }
                    else
                    {
                        def designerType = child.designerType;
                        def id = "autoGenerated"+(autoGenerateId++)
                        def childProps = [designerType: designerType, id:id]
