...
_syncColWidths : function() {
    ...
    if(this._elTbody.rowEls.length > 0) {
...
_initTheadEls : function() {
    ...
    // Reinitialization
        ...
        for(i=0; i<aTheads.length; i++) {
            for(j=aTheads[i].rowEls.length-1; j>-1; j--) {
                Ev.purgeElement(aTheads[i].rowEls[j], true);
                aTheads[i].removeChild(aTheads[i].rowEls[j]);
            }     
        }
    }
...
_deleteTrEl : function(row) {
    ...
    if(lang.isNumber(rowIndex) && (rowIndex > -2) && (rowIndex < this._elTbody.rowEls.length)) {
...
_setRowStripes : function(row, range) {
    ...
    var allRows = this._elTbody.rowEls,
        nStartIndex = 0,
...
getFirstTrEl : function() {
    return this._elTbody.rowEls[0] || null;
},
        ...
        this._oChainRender.add({
            method: function(oArg) {
                ...
                if(this._bInit) {
                    ...
                    this._oChainRender.add({
                        method: function() {
                            if((this instanceof DT) && this._sId && this._bInit) {
                                this._bInit = false;
                                this.fireEvent("initEvent");
                                YAHOO.log("DataTable initialized with " + allRecords.length + " of " + this._oRecordSet.getLength() + "rowElss", "info", this.toString());
                            }
                        },
                        scope: this
                    });
                    ...
                    YAHOO.log("DataTable rendered " + allRecords.length + " of " + this._oRecordSet.getLength() + "rowElss", "info", this.toString());
                }
...
_setColumnWidth : function(oColumn, sWidth, sOverflow) {
    ...
    if(oColumn) {
        ...
        if(DT._bStylesheetFallback) {           
            ...
            var rowslen = this._elTbody ? this._elTbody.rowEls.length : 0;
...
hideColumn : function(oColumn) {
    ...
    if(oColumn && !oColumn.hidden) {
        ...
        if(oColumn.getTreeIndex() !== null) {
            var allrows = this.getTbodyEl().rowEls;
...
_addTrEl : function(oRecord, index) {
    ...
    if(elNewTr) {
        if (index >= 0 && index < this._elTbody.rowEls.length) {
            this._elTbody.insertBefore(elNewTr,
                this._elTbody.rowEls[index]);
            if (!index) {
                this._setFirstRow();
            }
        } else {
            ...
            this._setLastRow();
            index = this._elTbody.rowEls.length - 1;
        }
                        ...
                        method: function() {
                            if((this instanceof DT) && this._sId) {
                                ...
                                // Update UI
                                    ...
                                    // Set EVEN/ODD
                                    if(nTrIndex != this._elTbody.rowEls.length) {
