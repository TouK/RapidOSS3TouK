...
<div class="body">
            ...
            <tbody>
                <g:each in="${rsEventList}" status="i" var="rsEvent">
                    <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                        ...
                        <td>${rsEvent.className?.encodeAsHTML()}</td>
                        
                        <td>${rsEvent.eventName?.encodeAsHTML()}</td>
                        
                        <td>${rsEvent.instanceName?.encodeAsHTML()}</td>
    ...
    <div class="paginateButtons">
        <g:paginate total="${RsEvent.countHits('id:[0 TO *]')}"/>
    </div>
</div>
</body>
</html>
