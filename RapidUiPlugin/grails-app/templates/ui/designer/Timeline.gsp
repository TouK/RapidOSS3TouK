<rui:timeline id="${uiElement.name}" url="${uiElement.url}" title="${uiElement.title}" pollingInterval="${uiElement.pollingInterval}">
    <rui:tlBands>
        ...
        %>
        <rui:tlBand width="${band.width}%" intervalUnit="${band.intervalUnit}" intervalPixels="${band.intervalPixels}" ${band.date > new Date(0)?"date=\""+dateFormatter.format(band.date)+"\"":""}
