...
class AolSenderScriptTests  extends RapidCmdbWithCompassTestCase {
    ...
    void initializeScriptManager()
    {
        ...
        FileUtils.copyFileToDirectory (new File(getWorkspacePath()+"/RapidModules/RapidInsight/scripts/aolSender.groovy"),scriptsDir);

        ScriptManagerForTest.addScript('aolSender');
    }
    ...
    void testSenderCallsRenderTemplateAndPassesTemplateResultToSendMessage()
    {
        ...
        events.each{ event ->
            RsMessage.add(eventId:event.id,destination:destination,destinationType:AOL_TYPE,eventType:RsMessage.ACTION_CREATE,state:RsMessage.STATE_READY);
        }
        ...
        historicalEvents.each{ event ->
            RsMessage.add(eventId:event.activeId,destination:destination,destinationType:AOL_TYPE,eventType:RsMessage.ACTION_CLEAR,state:RsMessage.STATE_READY);
        }
        ...
        ScriptManagerForTest.runScript("aolSender",[staticParamMap:[connectorName:connectorParams.name],logger:TestLogUtils.log])
        ...
        RsMessage.searchEvery("alias:*",[sort: "id",order:"asc"]).each{ message ->
           ...
           assertEquals("grails-app/templates/message/aolTemplate.gsp",renderTemplateParams[index].templatePath);
