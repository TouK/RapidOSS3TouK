...
class UploadFileActionTest extends RapidCmdbTestCase {
  ...
  def initConn() {
    ...
    conn.init(param);
    httpUtilsMock = new HttpUtilsMock(1000);
  ...
  public void testUploadFile() {
    ...
    HttpPost mt = executedPostMethods[0];
    assertEquals(new URI("${baseUrl}${actionUrl}").toString(), mt.getURI().toString());

    MultipartEntity entity = mt.getEntity();

    assertTrue(entity.getContentType().toString().indexOf("multipart/form-data;") >= 0);
