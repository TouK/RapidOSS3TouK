...
class RsUserController {
    ...
    def addUserWithRole = {
        ...
        if (password1 && password1 != "") {
            ...
            if (!role) {
                addError("default.null.message", ["role", UserRoleRel.class.getName()]);
                withFormat {
                    xml {render(text: errorsToXml(errors), contentType: "text/xml")}
                }
                return;
            }
            if (role != Role.ADMINISTRATOR && role != Role.USER) {
                addError("default.not.inlist.message", ["role", UserRoleRel.class.getName(), role, "${Role.ADMINISTRATOR}, ${Role.USER}"]);
                withFormat {
                    xml {render(text: errorsToXml(errors), contentType: "text/xml")}
                }
                return;
            ...
            def passHash = new Sha1Hash(password1).toHex();
            def rsUser = RsUser.add(username:params.username, passwordHash:passHash);
            if (!rsUser.hasErrors()) {
                def userRole = Role.get(name:role);
                UserRoleRel.add(rsUser:rsUser, role:userRole);
                withFormat {
                    xml {render(text: ControllerUtils.convertSuccessToXml("User ${params.username} successfully added."), contentType: "text/xml")}
                }
                return;
            }
            else {
